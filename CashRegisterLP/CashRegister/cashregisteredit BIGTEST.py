...
def prevent_option_input_item_to_change_list(is_input_variable_an_option):
    # Test if the string starts with "i"
    if is_input_variable_an_option.startswith('i'):
        if
            # Splice the first character
            first_character = is_input_variable_an_option[0]

            # Test if the rest of the string is an integer
            if my_string[1:].isdigit():
            print(f"The first character is '{first_character}' and the rest is an integer.")
    else:
    print(f"The first character is '{first_character}', but the rest is not an integer.")
