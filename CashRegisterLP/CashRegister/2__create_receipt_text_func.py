...
def create_receipt_text_func(_current_cart_): # take reciept_text_memoryies documented item list then ocmapre that to the impt 
    ...
    receipt_text_memory = receipt_text_memory[1]
    ...
    # waht i need to do is basically wait yeah thats it i did it. yeah ok now i want ot return hte receipt text memory baisally when its done
        ...
        #receipt_text_memory = []
        #receipt_text_memory.append(current_cart_unique_item_tuple)
        #receipt_text_memory.append(receipt_text)

        receipt_text_memory = [current_cart_unique_item_tuple, receipt_text]
...
# oh my god is makes sense when you use the like = sign you are basically creating a relationship between the 1 tuple and the other and the problem is tuples cant change
# taht makes so much sense and stirngs also cant so basically you have to delete the index then insert another object into that index
