...
def complete_program_loop(Main_menu_options, menu_num, default_l_h_and_min_l_h):
...
#{'navigation_to_non_existent' : True} seems not necesary
            ...
            str_empt_main_menu_box = main_same_top_and_bot_empty_str_box(length, height, ls_row_column_chars, box_peri_start_pos)
            str_options_column_plus_chars_between_titles = str_one_column_menu_options(length, height - 2, Main_menu_options, options_column_start_pos)
            ...
            while True:
                ...
                elif event_.name == 'enter': # we gotta create an iterable test solutio nfor this yeah that makes sense
                    if first_enter_press:
                        first_enter_press = False
                        pass
                    # 1. purchase
                    elif (sel_option[0] == 1 ) and not first_enter_press:
...
globals = {'size' : {'length' : [], 'height' : []}, 'mem' : 0, 'box_cursor_pos' : pos(1,1)}
...
while True:
    menu_num = complete_program_loop(Main_menu_options, menu_num, default_l_h_and_min_l_h)
    if menu_num == 'p':
        break
    elif menu_num == '/':
        menu_num -= 1
