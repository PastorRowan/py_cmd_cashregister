...
# commented version with tests

import mmap

def loads_and_removes_current_cart_item_s_data_to_memory(current_cart_): # what are we gonna call this we could call this unique items input becuase the current_cart is now a dioctioanry but i dont want
    ...
    if current_cart_ != {}:
        ...
        if cart_in_memory != current_cart_unique_item_set:
            item_data_in_memory['cart_in_memory'] = current_cart_unique_item_set
            item_data_in_memory = {key: value for key, value in item_data_in_memory.items() if key in current_cart_unique_item_set or key == 'cart_in_memory'}
        # item was memory but not in current cart and therefore was removed
            print(item_data_in_memory)
            input('item was in memory but not in current cart and therefore was removed')
            ...
            for unique_current_cart_item in current_cart_unique_item_set:
                ...
                elif unique_current_cart_item in item_data_in_memory:
...
item_data_in_memory = {'cart_in_memory' : set(), 'i1' : ('GTX1050_2GB', 3000.01, 21), 'i3': ('RX480_8GB', 4500.0, 25), 'i4': ('RX580_8GB', 4500.0, 30)} # are we gonna make the cart in ememory jsut aset of the unqiue items
# yeah why not
