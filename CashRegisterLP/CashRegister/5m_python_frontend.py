        ...
        if func_num == 1:
            ...
            def handle_left(new_l, min_l, new_h, min_h, box_curs_pos):
                if (new_l[0] - 1) > min_l:
                    ...
                    return {'new_l' : new_l, 'l_or_h_change' : 'l','reduce_min_l_attempt' : False, 'menu' : 'display_menu'}
                else:
                    return {'new_l' : new_l, 'l_or_h_change' : 'l','reduce_min_l_attempt' : True, 'menu' : 'display_menu'}
            ...
            def handle_up(new_l, min_l, new_h, min_h, box_curs_pos):
                if (new_h[0] - 1) > min_h:
                    ...
                    return {'new_h' : new_h, 'l_or_h_change' : 'h','reduce_min_h_attempt' : False, 'menu' : 'display_menu'}
                else:
                    return {'new_h' : new_h, 'l_or_h_change' : 'h','reduce_min_h_attempt' : True, 'menu' : 'display_menu'}

            def handle_down(new_l, min_l, new_h, min_h, box_curs_pos):
                new_h[0] += 1
                str_eg_box = str_eg_box_(new_l[0], new_h[0])
                print(box_curs_pos  + str_eg_box)
                return {'new_h' : new_h, 'l_or_h_change' : 'h', 'above_max_h_attempt' : False, 'menu' : 'display_menu'}
            ...
            def handle_slash(new_l, min_l, new_h, min_h, box_curs_pos):
                ...
                return {'menu' : 'main_menu'}
# ok hwat would be the best return value to test fr if yor return value is a dictioanry i mean what i could do is take the diitoanry and say if this is in his then do this or something but tbh i dont like that i
# would prefer to have a specific like thing like indicator that i put i na specfic postion that says ok this is the next function or somethig like almost like a dictioanry value pair the key is like menu  then
# the value is like main enu or purchacse menu and then if the ok i prefer that make it that every reutrn vaue has it and if its NOT the main function then go like ok what is it? if it is main menu then do this
# if it is this then do this or something like that. that makes sense to me
            handlers = {
            'left': handle_left,
            'right': handle_right,
            'up': handle_up,
