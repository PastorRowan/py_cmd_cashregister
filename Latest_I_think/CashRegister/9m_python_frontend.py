        ...
        if event_.event_type == keyboard1.KEY_DOWN:
            ...
            # only runs if up arrow or down arrow are pressed
                if ((event_.name == 'up') or (event_.name == 'down')):
                    ...
                    return ('left', sel_option_before_change, options[sel_option_before_change])
                    ...
                    str_updated_desel_title = Main_menu_main_str_update_sel_title(sel_option_before_change, column_start_x, column_start_y, options, total_options, chars_between_titles, foreg_colour)
                    print(str_updated_desel_title)
                    return ('right', sel_option_before_change, options[sel_option_before_change])


                elif (event_.name == 'enter'): # we gotta create an iterable test solutio nfor this yeah that makes sens 
                    return ('enter', sel_option_before_change, options[sel_option_before_change])
...
def complete_program_loop(global_vars):

    menu_num = -1

    start_menu = global_vars['start_menu']
    Main_menu_options = global_vars['Main_menu_options']
    size = global_vars['size']
    ls_menu_num = global_vars['ls_menu_num']
...
# consider using kwargs or args it really dpeends on the functions purpose and needs and future palnsof the proram 
                            ...
                            global_vars['size']['l'] = new_l
                            global_vars['size']['h'] = new_h
                            global_vars['ls_menu_num'][0] = 2
...
#{'navigation_to_non_existent' : True} seems not necesary
            ...
            # creates and prints main menu box
            str_empt_main_menu_box = main_same_top_and_bot_empty_str_box(ls_l[0], ls_h[0], ls_row_column_chars, box_peri_start_pos)
            str_options_column_plus_chars_between_titles = str_one_column_menu_options(ls_l[0], ls_h[0] - 2, Main_menu_options, options_column_start_pos)

            str_options_column = str_options_column_plus_chars_between_titles[0]
            chars_between_titles = str_options_column_plus_chars_between_titles[1]

            print(str_empt_main_menu_box + options_column_start_pos + str_options_column + options_column_start_pos + Fore.MAGENTA + Main_menu_options[1] + Style.RESET_ALL)
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            menu_block_info = {'start_and_end_coords' : {'start_x': 1, 'start_y': 1, 'end_x' : 51, 'end_y': 26},
                   ...
                   'size' : {'length' : ls_l[0], 'height' : ls_h[0]}}
            ...
            sub_boxes_start_and_end_coords = block_coords(ls_l[0], ls_h[0], boxes_start_and_end_percentages, peri_start_x, peri_start_y, peri_menu_end_x, peri_menu_end_y)
            ...
            # printing actual UI
            print(str_perfect_clear_box(ls_l[0], ls_h[0], peri_start_x + 1 , peri_start_y + 1) + Style.RESET_ALL)
            ...
            while True:
                ...
                if GUI_loop:
                    if current_sel_block[0] == 1:
                        GUI_return = GUI_loop(last_option_num, current_sel_block_info, item_searcher_options)

                        last_key_stroke = GUI_return[0]
                        last_option_num = GUI_return[1]

                    elif current_sel_block[0] == 2:
                        GUI_return = GUI_loop(last_option_num, current_sel_block_info, cart_items)
                        ...
                        last_key_stroke = GUI_return[0]
                        last_option_num = GUI_return[1]
                        last_select_option = GUI_return[2]

                if GUI_return == None:
                    continue

                elif (((last_key_stroke == 'left') or (last_key_stroke == 'right')) and (current_sel_block[0] == 1)) and (GUI_return != None):
                    current_sel_block[0] += 1
                    new_sel_current_block_str_id = blocks.get(current_sel_block[0])
                    current_sel_block_info = sub_boxes_info[new_sel_current_block_str_id]

                elif (((last_key_stroke == 'left') or (last_key_stroke == 'right')) and (current_sel_block[0] == 2)) and (GUI_return != None):
                    current_sel_block[0] -= 1

                    new_sel_current_block_str_id = blocks.get(current_sel_block[0])
                    current_sel_block_info = sub_boxes_info[new_sel_current_block_str_id]

                elif ((last_key_stroke == 'enter') and (current_sel_block[0] == 1)) and (GUI_return != None):
                    
                    order_num = len(cart_items) + 1
                    cart_items[order_num] = last_select_option
                    # we need to create a function where basically now when you press enter it takes that current selectded option and then adds it into a dictionary like 1 : last option name or something
                    # like that so that then we have a diitonary that has all the cart titles and the order they were dropped into the cart
...
while True:
    global_vars = complete_program_loop(global_vars)
