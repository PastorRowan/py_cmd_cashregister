...
def create_receipt_text_func_old(input_cart): # take reciept_text_memoryies documented item list then ocmapre that to the impt 
    float_total = 0.
    ...
    if (input_cart != []) and (input_cart != receipt_text_in_memory):
        receipt_text_list = []
        unique_item_list = [] # could make tuple to save memory??? liek waht we did in
