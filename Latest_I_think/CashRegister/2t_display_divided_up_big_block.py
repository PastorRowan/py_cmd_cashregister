...
if True:
    if True:
        def GUI_item_searcher_block_updater(start_option, box_info, options):
            ...
            print(column_start_pos + str_options_column + column_start_pos + Fore.MAGENTA + options[1])
            ...
            for _ in range(2):
                while True:
                    ...
                    key_pressed_TorF = False

                    if event_.event_type == keyboard1.KEY_DOWN:
                        key_pressed_TorF = True
                    ...
                    if ((event_.name == 'up') or (event_.name == 'down'))and key_pressed_TorF:
                    ...
                    elif (event_.name == 'left') and key_pressed_TorF:
                        return ('left', sel_option_before_change)
                    
                    elif (event_.name == 'right') and key_pressed_TorF:
                        return ('right', sel_option_before_change)


                    elif (event_.name == 'enter') and key_pressed_TorF: # we gotta create an iterable test solutio nfor this yeah that makes sens 
...
while True:
    ...
    if GUI_loop:
        GUI_return = GUI_loop(current_sel_block[0], current_sel_block_info, item_searcher_options)
