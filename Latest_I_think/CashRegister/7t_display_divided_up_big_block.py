...
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
str_options_column_plus_chars_between_titles
# titles
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
# ok one way thissyetm could work is that if we were now to basically have like idk lets say we had like percentages like ok this is block 1 and block 2 and whatever if you have 25% down block 1 then you will
# hit the option that is 25% down block 2 and this will change depending on the postion of said boxes but honestly a way more simple system would be like ok we are gonna have x options here then the same amount of
# options here (x options here) then we can take the options number ad then uust go like ok we are on option num 1 in htis colum so we go to option number 2 in the other column and then yeah. but the problem is
# gonna come when like ok we have one block that only has 2 options next to another block that has 8 or something like that then what do we do? hmmm idk. then we coudl use the percentage rule? but then if we go
# back then its ognna start at the top again? hmmmmmm i guess what we could do for that is just the percentage method would work but then also we gotta create a theme for our cash register i thnk thats also mportant
# i dont want it to just be a boring 
...
# ok so we need to find the pos of the selected option then bascially take that pos and then replacew that with column start pos then repalce
    print(column_start_pos + str_options_column)
...
# i could create a first setup like varible taht makes it taht when the loop is calle dit skips over the intial setup sequencew of hte code and isntead goes straght to the GUI loop so y eah but i think that 
